			...
			case EntityDefinitionType::Citizen:
			{
				if (passesNpcDistanceTest && canPlayerMoveAndTurn)
				{
					...
					else if (interactionType == GameWorldInteractionType::Thieving)
					{
						...
						bool pickPocketSuccess = ArenaPlayerUtils::attemptPickPocket(cityType, charClassDef.thievingDivisor, player.level, player.primaryAttributes, arenaRandom, &pickpocketResultTemplateDatIndex, &pickpocketGoldAmount, &guardsAppear);
